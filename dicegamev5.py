...
# This program is very complex to debug, I may not be able to see all
...
def dicegame_saves(fst_name, playerScore, di_max): # File export starts here
    Lin_or_Win = 'L'
    ...
    while file_exp_flag != 'y':
        if file_exp_flag == 'c' or file_exp_flag == '': break
        #adv. while loop (if break combination) was used as both while-or; while-and are bad
    ...
    if file_exp_flag == 'y' and exp_path == 'TBD': #2nd posibility of path change --- desktop not found
        ...
        print('\nFile '+full_exp_path+' export successful. Please run this module again for another match.')
        files.close()

    else: print('\nNo file was exported.\nPlease run this module again for another match.')

#shorten random, default highest dice count is 6
def di(di_max):
    return random.randint(1, di_max)

def mechs_dsply(): print('''----------------Welcome to the dice game----------------\n
...
def dicegame(fst_name, alt_di_max):
    #input first name, input highest dice count
    mechs_dsply() #show beginning text
    #permenent variable initialize
    playerScore = turn = 0
    pCountDown = 3
    #register player and initialize highest dice count
    ...
    di_max = 6
    while alt_di_max != 'y': #Reject false input: not 'y', but not ''
        if alt_di_max == '': break 
        #adv. while loop (if break combination) was used as both while-or; while-and are bad
    ...
    while turn < 5: #Game starts here
        diRolls = [di(di_max),di(di_max),di(di_max),di(di_max),di(di_max)]
    ...
    dicegame_saves(fst_name, playerScore, di_max)

dicegame(fst_name = str(input('What\'s your first name? ')), alt_di_max = str(input('\nEnter y to alter the highest dice count can be rolled\nDefault=6, higher is harder. Or press Enter to skip. (y/Enter): ')))